...
describe "store" do
  ...
  it "add item twice overrides previous data" do
    store = Store.new
    store.add_item("001", "apple", 10)
    store.add_item("001", "orange", 20)
    assert_equal("orange", store.get_item("001").name)
    assert_equal(20, store.get_item("001").price)
  end
